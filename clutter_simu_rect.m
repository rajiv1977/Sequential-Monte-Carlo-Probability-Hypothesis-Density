...
%                                                                                           %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function clutter_out=clutter_simu_rect(area_num,simu_range,simu_num,data_len,clut_num,para)
%assume the shape of each high clutter density area is rectangular
%area_num, the number of high clutter area
%simu_range, the total area of surveillance area, assume its shape is
...
%(2) for each row: percentenge of clutter number,x_min,x_max,y_min,y_max
...
for k=1:simu_num
    for i=1:data_len
        ...
        dump_index=1;
        all_num=2*clut_num;
        while (all_num>1.5*clut_num)|(all_num<0.5*clut_num)
        ...
        if area_num>=1
            for j=1:area_num
                ...
                if temp_num>=1
                    for m=1:temp_num
                        temp_data(:,dump_index)=para(j,2)+(para(j,3)-para(j,2))*rand;
